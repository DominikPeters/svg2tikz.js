...
export function svgToTikz(svgInput, options = {}) {
    const { precision = 2, scale = null, standalone = false, } = options;
...
function parseTransformAttr(str) {
    if (!str)
        return { matrix: identityTransform(), ops: [] };
...
function buildDrawCommand(el, style, ctx) {
    ...
    if (hasGradient) {
        ...
        if (hasFill)
            fillColor = getTikzColor(fillHex, ctx);
    }
...
function appendStrokeAndOpacityOptions(opts, style, ctx) {
    ...
    if (dashPattern)
        opts.push(`dash pattern=${dashPattern}`);
    ...
    if (linecap)
        opts.push(`line cap=${linecap}`);
    const linejoin = mapLinejoin(style.strokeLinejoin);
    if (linejoin)
        opts.push(`line join=${linejoin}`);
}
...
function buildTransformScopes(transform, ctx, forNode) {
    ...
    if (opts == null)
        opts = buildCmTransformOpts(transform, ctx);
    ...
    switch (op.type) {
        ...
        case 'scale': {
            ...
            if (Math.abs(sx - sy) < EPSILON) {
                ...
                if (Math.abs(sx - 1) >= EPSILON)
                    scaleOpts.push(`xscale=${fmt(sx, ctx)}`);
                if (Math.abs(sy - 1) >= EPSILON)
                    scaleOpts.push(`yscale=${fmt(sy, ctx)}`);
                if (!scaleOpts.length)
                    return [];
            }
...
function mergeShiftSteps(a, b) {
    ...
    if (Math.abs(x) >= EPSILON)
        result.push(`xshift=${trimFloat(x)}cm`);
    if (Math.abs(y) >= EPSILON)
        result.push(`yshift=${trimFloat(y)}cm`);
    return result;
}
...
function wrapPivotedTransform(pivot, transformOpts, ctx) {
    ...
    if (startShift.length)
        scopes.push(startShift);
    scopes.push(transformOpts);
    if (endShift.length)
        scopes.push(endShift);
    return scopes;
}
function buildShiftOpts(x, y, ctx) {
    const opts = [];
    if (Math.abs(x) >= EPSILON)
        opts.push(`xshift=${fmt(x, ctx)}cm`);
    if (Math.abs(y) >= EPSILON)
        opts.push(`yshift=${fmt(y, ctx)}cm`);
    return opts;
}
...
function emitRect(el, ctx, style, transform) {
    ...
    if (radius != null && radius > 0)
        opts.push(`rounded corners=${dimPt(radius, ctx)}`);
    ...
    const render = () => {
        withTransformScopes(ctx, useTransform, true, () => {
            if (target.tagName?.toLowerCase() === 'g')
                processChildren(target, ctx);
            else
                processChildren(wrapElement(target), ctx);
        });
    };
    if (opacity != null && opacity < 1) {
...
function parseTextCoord(el, attr) {
    ...
    if (own)
        return parseNumeric(own.split(/[\s,]+/)[0]);
...
function applyInlineTextStyles(content, style, inheritedStyle, rootStyle) {
    ...
    if (baseline.includes('super'))
        result = `\\(^{${result}}\\)`;
    else if (baseline.includes('sub'))
        result = `\\(_{${result}}\\)`;
    return result;
}
...
function colorToHex(colorStr) {
    ...
    if (!color || color === 'none' || color === 'transparent')
        return null;
    if (CSS_COLOR_KEYWORDS.has(color))
        return CSS_COLOR_KEYWORDS.get(color);
    ...
    if (typeof document !== 'undefined') {
        ...
        if (context) {
            ...
            if (normalized === '#000000' && color === 'black')
                return '000000';
        }
    }
    return null;
...
function getTikzColor(hex, ctx) {
    if (!hex)
        return null;
    if (XCOLOR_EXACT.has(hex))
        return XCOLOR_EXACT.get(hex);
    if (ctx.colors.has(hex))
        return ctx.colors.get(hex);
...
//# sourceMappingURL=svg2tikz.js.map
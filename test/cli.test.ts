...
test('cli writes standalone output to a file', async () => {
  ...
  await fs.writeFile(
    inputPath,
    `<svg viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><circle cx="10" cy="10" r="4" fill="none" stroke="#0000ff"/></svg>`,
    'utf8'
  );

  await execFileAsync(
    process.execPath,
    [cliPath, inputPath, '--output', outputPath, '--standalone', '--precision', '3'],
    {
      cwd: path.resolve('.'),
      maxBuffer: 1024 * 1024,
    }
